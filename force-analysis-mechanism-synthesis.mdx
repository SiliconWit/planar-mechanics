...
1. **Open the simulator** ([siwit.co/FBL](https://siwit.co/FBL)), set $a = 40$, $b = 120$, $c = 80$, $d = 100$, and read the transmission-angle trace: peak about $86\degree$, minimum about $26\degree$. ✅

2. **Cross-check against mechanical advantage.** The mechanical-advantage trace should collapse at exactly the crank angles where $\mu$ dips, because both are measuring the same loss of leverage from two directions. ✅
...
1. **Open the simulator** ([siwit.co/SLM](https://siwit.co/SLM)), set the platform load to $500$ N and the actuator type to **Horizontal Base**, then read the actuator-force trace at $\theta = 45\degree$, $30\degree$ and $10\degree$. They should land on $500$, $866$ and about $2840$ N. ✅

2. **Change the actuator placement.** Switch the actuator type to **Diagonal Base** and watch the low-angle end of the curve drop: the geometry factor has changed, so $\cot\theta$ no longer describes it, and the trade-off between the two ends of the stroke becomes visible. This is the mounting choice that makes low-angle work practical. ✅

3. **Check the link stress.** With the actuator force known, the arm stress follows as in Application 1. The lowest angles are the worst case for both force and stress at once, which is why a lift is rated from its lowest working height. ✅
