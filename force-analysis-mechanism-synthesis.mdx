...
<CardGrid>
  ...
  The force ratio is the reciprocal of the [velocity ratio](./velocity-analysis-instantaneous-centers). Motion and force are two views of one machine.
  </Card>
...
The mechanical advantage is the **reciprocal of the velocity ratio** found in the [velocity analysis](./velocity-analysis-instantaneous-centers). Where the output slows (a limit or toggle position, velocity ratio toward zero), the mechanical advantage grows large. This is why the same toggle position that stopped the output in the velocity analysis amplifies force here. Real mechanisms lose a little to friction, so an efficiency $\eta < 1$ multiplies the ideal value.
</Card>
...
Once a joint force $F$ is known, a **pin** of diameter $d$ in single shear carries $\tau = F/(\pi d^2/4)$, and a rectangular **link** of thickness $t$ and width $w$ under a bending moment $M$ carries $\sigma = M/Z$ with $Z = t w^2/6$. Both must stay under the **allowable stress** $\sigma_\text{all} = \sigma_\text{yield}/N$ for a safety factor $N$.
</Card>
...
This is the capstone worked example: from hand force to clamping force by force polygon, then the pin and link stresses. The force polygon is the same [graphical method](./position-analysis-planar-linkages#methods-of-kinematic-analysis) used for velocities and accelerations, now applied to forces: draw the balance to scale, solve it exactly, and confirm in the simulator.
...
1. **Mechanical advantage** at the $5\degree$ lock margin:

   $$\text{MA}_\text{ideal} = \frac{1}{2\tan 5\degree} = 5.72, \qquad \text{MA}_\text{real} = \eta\,\text{MA}_\text{ideal} = 0.75(5.72) = 4.3$$ ✅

2. **Clamping force:**

   $$F_\text{clamp} = \text{MA}_\text{real}\times F_h = 4.3 \times 100 = 430 \text{ N}$$ ✅
...
3. **Stress check** at a representative link/pin force of $800$ N (the internal forces exceed the pad force near the joints):

   $$\tau_\text{pin} = \frac{800}{\pi(8)^2/4} = 15.9 \text{ MPa}, \qquad \sigma_\text{link} = \frac{6 F L_\text{arm}}{t w^2} = \frac{6(800)(40)}{6(20)^2} = 80 \text{ MPa}$$ ✅

   With allowable $\sigma_\text{all} = 350/2 = 175$ MPa, both are well within limits. ✅
...
The toggle clamp is the whole course in one device: a 1-DOF four-bar ([mobility](./kinematic-joints-constraint-analysis)), positioned at top-dead-centre ([position](./position-analysis-planar-linkages)), where the velocity ratio vanishes ([velocity](./velocity-analysis-instantaneous-centers)), so the mechanical advantage and clamping force spike here in the force analysis, and the resulting pin and link forces must be carried without yielding.
:::
...
:::note[Engineering Insight]
The transmission angle is a force-quality gauge that costs nothing to check: it comes straight from the [position analysis](./position-analysis-planar-linkages). Keeping it above about $40\degree$ through the working stroke is one of the first rules of good linkage design.
:::
...
<CardGrid>
  ...
  Use the [velocity ratio](./velocity-analysis-instantaneous-centers): mechanical advantage is its reciprocal. No separate force derivation is needed for the ideal value.
  </Card>
...
|-----------|--------------------|--------------|-----------|
| Toggle clamp | clamping force, stresses | $\text{MA} = 1/(2\tan\alpha)$ | [siwit.co/TCM](https://siwit.co/TCM) |
