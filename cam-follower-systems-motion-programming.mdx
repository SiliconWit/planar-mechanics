...
Every peak below has the same shape: a pure number times $h/\beta$ for velocity, and a pure number times $h/\beta^2$ for acceleration. Those pure numbers are the **motion-law factors** $C_v$ and $C_a$. They depend only on the *law*, never on the rise height, the rise angle or the cam speed, which is what makes them worth memorising.

$$v_\text{max} = C_v\,\frac{h}{\beta}, \qquad a_\text{max} = C_a\,\frac{h}{\beta^2}$$

| Motion law | $C_v$ | $C_a$ | Acceleration at the ends | Suitability |
|------------|:-----:|:-----:|--------------------------|-------------|
| Uniform (constant velocity) | $1.000$ | $\to\infty$ at the ends | infinite velocity jump | only with rounded ends |
| Parabolic (constant accel) | $2.000$ | $4.000$ | jumps (finite, but step) | low speed |
| Simple harmonic (SHM) | $\pi/2 = 1.571$ | $\pi^2/2 = 4.935$ | jumps from zero at a dwell | moderate speed |
| Cycloidal | $2.000$ | $2\pi = 6.283$ | zero at both ends | high speed, smoothest |
...
:::tip[Read the units off the factor]
$C_v$ and $C_a$ are dimensionless, so $C_v h/\beta$ carries the units of $h$ per **radian** of cam angle, and $C_a h/\beta^2$ per radian squared. $\beta$ must therefore be in radians, never degrees, before you divide. To reach millimetres per **second** you multiply by $\omega$; for mm/s² multiply by $\omega^2$. Keeping the factor and the speed separate means one calculation covers every cam speed.
:::
</Card>
...
For a **roller** follower, replace $R_b$ with the prime-circle radius $R_p = R_b + R_r$, where $R_r$ is the roller radius; the contact geometry is set by the roller centre path, not the cam surface.

**Where the maximum falls.** The numerator $ds/d\theta$ peaks at mid-rise while the denominator keeps growing, so the true maximum of $\phi$ sits slightly *before* mid-rise. Evaluating at mid-rise is close enough for design and gives a formula you can use by hand. For a cycloidal rise, $s = h/2$ and $ds/d\theta = 2h/\beta$ there, so:

$$\tan\phi_\text{max} \approx \frac{2h/\beta}{R_b + h/2}$$

This runs about half a degree below the true peak, so treat it as a slight under-estimate and keep a little margin against the $30\degree$ guide. Turning it around gives the base circle a specification demands:

$$R_b \ge \frac{2h/\beta}{\tan\phi_\text{allow}} - \frac{h}{2}$$
</Card>
...
This is the central worked example. We program a rise and draw its displacement, velocity, and acceleration to scale, the graphical heart of cam design. It follows the same [draw, solve, simulate](/education/planar-mechanics/position-analysis-planar-linkages#methods-of-kinematic-analysis) rhythm as the linkage lessons: draw the motion to scale, find its peaks analytically, then confirm in the simulator.
...
- **Input:** constant cam speed $\omega = 30$ rad/s (about 287 rev/min)
...
1. **Convert the rise angle to radians first.** $\beta = 90\degree = \pi/2 = 1.5708$ rad. Every factor formula divides by $\beta$, so a rise angle left in degrees is the single most common way to lose these marks. ✅

2. **Write down the factors** for the cycloidal law: $C_v = 2.000$ and $C_a = 2\pi = 6.283$. ✅

3. **Peak velocity per unit cam angle**, at mid-rise ($\theta = \beta/2$):

   $$v_\text{max} = C_v\frac{h}{\beta} = \frac{2(25)}{1.5708} = 31.8 \text{ mm/rad}$$ ✅

4. **Peak acceleration per unit cam angle**, at the quarter points:

   $$a_\text{max} = C_a\frac{h}{\beta^2} = \frac{6.283(25)}{(1.5708)^2} = 63.7 \text{ mm/rad}^2$$ ✅

5. **Convert to real units at the cam speed** $\omega = 30$ rad/s. The per-radian figures are geometry alone; the speed enters only here:

   $$v_\text{max} = 31.8 \times 30 = 955 \text{ mm/s} = 0.955 \text{ m/s}$$

   $$a_\text{max} = 63.7 \times 30^2 = 57{,}300 \text{ mm/s}^2 = 57.3 \text{ m/s}^2$$ ✅

6. **Sanity-check the size of the answer.** $57.3$ m/s² is about $5.8g$. A follower of mass $0.2$ kg therefore needs roughly $11.5$ N just to be accelerated, before any spring or load force. Doubling the cam speed would quadruple it, which is exactly why acceleration, not velocity, sets the limit on how fast a cam can run. ✅
...
1. **Set it up, and set a speed.** Open the simulator ([siwit.co/CFM](https://siwit.co/CFM)), choose a cycloidal rise of $h = 25$ mm over $\beta = 90\degree$ with a knife-edge follower, and set the cam speed to **290 rpm**, the nearest step to the $\omega = 30$ rad/s used above. That gives $\omega = 30.4$ rad/s. ✅

2. **Expect real units, not per-radian ones.** The readout is **Peak velocity (at speed)**, in mm/s, and **Peak acceleration** in mm/s². The per-radian figures of Step 2 are pure geometry and never appear on screen; multiply them by $\omega$ and $\omega^2$ first: ✅

   | Quantity | Geometry only | Panel reads at 290 rpm |
   |---|---|---|
   | Peak velocity | $31.8$ mm/rad | $967$ mm/s |
   | Peak acceleration | $63.7$ mm/rad² | $58{,}700$ mm/s² |
...
3. **Find where each peak sits.** Go back to the normalised diagram of Step 1 and read the cam-angle axis: velocity peaks at mid-rise ($\theta/\beta = 0.5$) and acceleration at the quarter points ($0.25$ and $0.75$). Acceleration passes through zero at both ends of the rise, which is why the panel reports **Fundamental law** as obeyed. ✅

4. **Break the law deliberately.** Switch the rise to simple harmonic. Peak acceleration falls to about $50$ mm/rad², *lower* than cycloidal, yet the acceleration now steps at the dwell boundary and the panel flags the violation. Lower peak, worse cam: it is the discontinuity, not the magnitude, that breaks a high-speed follower. ✅
...
4. **The offset is not always possible.** Offsetting inward only works while the roller is smaller than the profile's tightest curve. Where the pitch curve is convex with radius of curvature $\rho$, a roller of radius $r_r$ cuts into the flanks either side once $r_r$ approaches $\rho$, and the cam can no longer deliver the motion you programmed. This is **undercutting**, and the usual working rule is $\rho_{min} > 1.5\,r_r$ or so. It is checked in Step 3, not assumed. ✅
...
<details>
<summary>**Click to reveal the profile and undercutting check**</summary>

<Steps>

1. **Rebuild the same cam.** Open the simulator ([siwit.co/CFM](https://siwit.co/CFM)), choose **cycloidal** for both rise and fall, and set the program of Application 1: lift $25$ mm, rise $90\degree$, dwell $90\degree$, fall $90\degree$, dwell $90\degree$, base radius $45$ mm. Tick **Show pitch curve**. ✅

2. **Check the two radii you drew.** The pitch curve should sit at a constant $45$ mm through the low dwell and a constant $70$ mm through the high dwell, with the rise and fall as the shaped transitions between the two arcs. Those are the same numbers as Step 2, read off a different construction. ✅

3. **Watch the roller offset appear.** Set the follower type to **Roller** and increase the roller radius from $10$ mm. The cam surface pulls inward from the pitch curve by exactly the roller radius, which is the offset you were told to apply by hand. ✅

4. **Push it until it fails.** Read **Min radius of curvature**: for this program it is about $36$ mm. Keep raising the roller radius and watch the ratio to $\rho_{min}$ close up, until the **Undercut** result reports the profile is no longer usable. A $10$ mm roller is comfortable here ($\rho_{min}/r_r \approx 3.6$), $20$ mm is still workable, and by $30$ mm you are into trouble. ✅
...
1. **Check the first guess by hand.** Use the mid-rise form $\tan\phi_\text{max} \approx (2h/\beta)/(R_b + h/2)$, with $2h/\beta = 31.8$ mm/rad from Application 1. At $R_b = 30$ mm:

   $$\tan\phi_\text{max} \approx \frac{31.8}{30 + 12.5} = 0.749 \;\Rightarrow\; \phi_\text{max} \approx 36.8\degree$$

   A full numerical sweep of the rise gives $37.8\degree$, so the hand estimate sits about a degree low, as expected. Either way it is well above the $30\degree$ guide and the follower would jam. ✅

2. **Size the base circle directly** rather than guessing again. Rearranging for $\phi_\text{allow} = 30\degree$:

   $$R_b \ge \frac{31.8}{\tan 30\degree} - 12.5 = 55.1 - 12.5 = 42.6 \text{ mm}$$

   Because the hand form under-estimates by about a degree, round up rather than taking $42.6$ mm as final. ✅

3. **Confirm the chosen size.** At $R_b = 45$ mm the hand form gives $\phi_\text{max} \approx 29.0\degree$ and the full sweep gives $29.4\degree$, just inside the guide with a little margin. This is why Application 2 used $R_b = 45$ mm. ✅

4. **Read the trade-off** from the plot below: the larger base circle lowers the whole pressure-angle curve under the $30\degree$ line, at the cost of a physically larger cam. Note the direction of the lever, the base circle buys pressure angle back cheaply at small $R_b$ and expensively at large $R_b$, so there is little point oversizing far beyond the guide. ✅
...
| Motion-law factors | $v_\text{max} = C_v h/\beta$, &nbsp; $a_\text{max} = C_a h/\beta^2$ &nbsp; ($\beta$ in radians) |
| Factors: cycloidal / SHM / parabolic | $C_v = 2.000$ / $1.571$ / $2.000$; &nbsp; $C_a = 6.283$ / $4.935$ / $4.000$ |
| Peaks at cam speed $\omega$ | $v_\text{max}\times\omega$ (mm/s), &nbsp; $a_\text{max}\times\omega^2$ (mm/s²) |
| Pressure angle | $\tan\phi = (ds/d\theta)/(R_b + s)$; roller: use $R_p = R_b + R_r$ |
| Max pressure angle (cycloidal) | $\tan\phi_\text{max} \approx (2h/\beta)/(R_b + h/2)$, slightly low |
| Base circle for a given $\phi_\text{allow}$ | $R_b \ge (2h/\beta)/\tan\phi_\text{allow} - h/2$ |
