...
**Applications:** [Cam-follower systems](./cam-follower-systems-motion-programming), gear-tooth contact, wheel rolling on a surface, ball bearings.
</Card>
...
2. **Interpret the result:**
   ...
   - With one input fixed, the coupler and follower angles are completely determined by the geometry (you solve for them explicitly in the [position analysis](./position-analysis-planar-linkages)). ✅
...
**Practical Impact:** This equivalence lets designers swap rotary and linear elements (a pin-jointed rocker for a hydraulic cylinder, say) without disturbing the degree-of-freedom budget. You will use exactly this freedom when sizing actuators in the [force analysis](./force-analysis-mechanism-synthesis).
:::
...
<CardGrid>
  ...
  **Best for:** programmed motion profiles and timed motion ([cam design](./cam-follower-systems-motion-programming)), speed/torque conversion.
...
Next, [Position Analysis of Planar Linkages](./position-analysis-planar-linkages) takes the four-bar linkage whose mobility we just confirmed and solves for the exact position of every link using vector loop equations, the foundation for the velocity, acceleration, and force analysis that follows.
