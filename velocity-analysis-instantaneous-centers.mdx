...
1. **Rod angular velocity** from differentiating $\phi = \arcsin((r/l)\sin\theta)$:
...
2. **A one-step check.** The rod's instantaneous center is where the crank line and the vertical through the piston cross, the same intersection that fixed $b$ in the polygon; dividing $v_A$ by the distance from that center to $A$ reproduces the same $\omega_3$. It is a quick confirmation, not the working method. ✅
...
2. **Velocity of the rod's mid-point $G$.** The mass centre sits at the middle of $AB$, so its image $g$ is the middle of $a\,b$. Measuring $o\,g$ gives $v_G \approx 48.7$ mm/s, the velocity the rod's inertia force will use in the [acceleration analysis](./acceleration-analysis-dynamic-forces). ✅
...
4. **Rubbing velocity at a pin.** At a pin joint the two links turn at different rates, so the journal surface rubs at $v_\text{rub} = (\text{relative angular velocity}) \times (\text{pin radius})$. With $\omega_2 = 1$ rad/s and $|\omega_3| = 0.174$ rad/s, and the journal radii shown:

   | Pin | Links joined | Relative $\omega$ (rad/s) | Radius (mm) | $v_\text{rub}$ (mm/s) |
   |---|---|:---:|:---:|:---:|
   | Main bearing $O$ | crank, frame | 1.000 | 25 | 25.0 |
   | Crank pin $A$ | crank, rod | 1.174 | 20 | 23.5 |
   | Gudgeon pin $B$ | rod, piston | 0.174 | 15 | 2.6 |

   The crank pin rubs fastest because the crank and rod turn in **opposite** senses there, so their rates add ($1 + 0.174$); the gudgeon pin rubs slowest because only the rod's small $\omega_3$ acts. Multiplying each rubbing velocity by the friction force at that pin gives the power lost there. ✅
...
4. **Why it matters.** The ram speed varies through the stroke, and it is deliberately low and steady over the cutting pass and high on the return. The [acceleration analysis](./acceleration-analysis-dynamic-forces) takes the next step: because the block slides while the lever turns, its acceleration carries a **Coriolis** term with no counterpart in the plain slider-crank. ✅
